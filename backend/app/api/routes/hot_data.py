...
from app.services.finnhub_hot import build_hot_stocks_by_market

router = APIRouter()


@router.get("/stocks/hot", response_model=HotStocksResponse)
async def get_hot_stocks(market: str = "us"):
    if not settings.finnhub_api_key:
        raise HTTPException(500, "Missing finnhub_api_key")
    now = datetime.now(timezone.utc)
    stocks = await build_hot_stocks_by_market(market, limit=30)
